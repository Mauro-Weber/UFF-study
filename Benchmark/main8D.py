...
from treatedDataframe8D import treatedDataFrame
...
def main():
    ...
    for cpu in range(4,8):
        ...
        .config("spark.task.cpus", f"{cpu+1}") \
        .getOrCreate()
        ...
        df = treatR[0]
        oqDF = df.sample(False, 0.000055, seed=12)
        df = df.subtract(oqDF) 
        size = treatR[1]
        ...
        for pivots in list_pivot:
            ...
            for method in pivots_method:
                if method == "random":
                    random_pivots_list = [(i+1,[np.random.rand(1)[0],\
                                                np.random.rand(1)[0],\
                                                np.random.rand(1)[0],\
                                                np.random.rand(1)[0],\
                                                np.random.rand(1)[0],\
                                                np.random.rand(1)[0],\
                                                np.random.rand(1)[0],\
                                                np.random.rand(1)[0]]) for i in range(pivots)]
        ...
        df_final.coalesce(1).write.csv(f"/home/weber/Documents/df100K_core8_cpu{cpu}.csv",header=True, mode="overwrite")
