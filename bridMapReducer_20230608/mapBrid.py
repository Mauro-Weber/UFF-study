    ...
    def process_partition(iterator):
        ...
        for row in iterator:         
            ...
            if not_stop_iteration == True:
                ...
                min_lower_bound = row[f"next_lb_{oq_id}"]
                
                if min_lower_bound is None:
                    continue
                
                if (distance.euclidean(oq, row.fv) < min_lower_bound) and (distance.euclidean(oq, row.fv) < queue_pq[0][0]):
                    influence_list.append([row.id,distance.euclidean(oq, row.fv),row.fv,row[f"next_lb_{oq_id}"]])
                    add_neighbor += 1

                else:
                    heapq.heappush(queue_pq, (distance.euclidean(oq, row.fv),row.id,row.fv,row[f"next_lb_{oq_id}"]))
